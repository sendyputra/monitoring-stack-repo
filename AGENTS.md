...
- `telegraf/` (stub_status + host collectors + access-log tailer) keeps the local Nginx/NPMplus dashboard healthy; `telegraf/npmplus-telegraf.conf` is the sample config when you deploy Telegraf on a remote NPMplus host.
